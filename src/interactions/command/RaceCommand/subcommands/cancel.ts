...
export default class RaceCancelSubcommand extends SubcommandInteraction {
    public info = new SlashCommandSubcommandBuilder()
        .setName("cancel")
        .setDescription("Cancels a race")
        .addStringOption(option => option
            .setName("race")
            .setDescription("The race that you want to cancel")
            .setRequired(true)
    ...
    public async execute(interaction: ChatInputCommandInteraction, client: DiscordClient): Promise<void> {
        ...
        try {
            ...
            console.error(e);
            await interaction.editReply(`Something went wrong when trying to cancel the race. Please promptly yell at Grass Wonder.`);
        }
    }
